...
{
    public class ImprovedScoringEngineService : IScoringEngine
    {
        public readonly IIndicatorRepositoryService _indicatorService;
        private readonly ILogger<ScoringEngineService> _logger;
        public ImprovedScoringEngineService(IIndicatorRepositoryService indicatorService, ILogger<ScoringEngineService> logger)
        {
        ...
        private bool EvaluateCriteria(CriteriaNode node, Household_Document doc, Member_Document? member = null)
        {
            if (!string.IsNullOrEmpty(node.Operator) && node.Rules != null)
                return EvaluateComposite(node, doc, member);

            if (!string.IsNullOrEmpty(node.FieldName) && !string.IsNullOrEmpty(node.Op))
                return EvaluateLeaf(node, doc, member);

            return false;
        }

        private bool EvaluateComposite(CriteriaNode node, Household_Document doc, Member_Document? member = null)
        {
            ...
            if (op == "AND")
            {
                // In member scope, keep evaluating against that same member.
                if (member != null)
                    return node.Rules.All(r => EvaluateCriteria(r, doc, member));

                // At household scope, if this AND node includes member-level predicates,
                // all of those member predicates must match the same member.
                var memberRules = node.Rules.Where(IsMemberScopedRule).ToList();
                if (memberRules.Count == 0)
                    return node.Rules.All(r => EvaluateCriteria(r, doc, null));

                var householdRules = node.Rules.Where(r => !IsMemberScopedRule(r)).ToList();
                if (!householdRules.All(r => EvaluateCriteria(r, doc, null)))
                    return false;

                return (doc.Members ?? Enumerable.Empty<Member_Document>())
                    .Any(m => memberRules.All(r => EvaluateCriteria(r, doc, m)));
            }
            if (op == "OR")
            {
                return node.Rules.Any(r => EvaluateCriteria(r, doc, member));
            }

            return false;
        }

        private bool EvaluateLeaf(CriteriaNode node, Household_Document doc, Member_Document? member = null)
        {
            ...
            var documentValues = GetFieldValues(doc, fieldName, member);
        ...
        private List<object?> GetFieldValues(Household_Document doc, string fieldName, Member_Document? member = null)
        {
            ...
            if (surveyMatch.Success)
            {
                ...
                var targetMembers = member != null
                    ? Enumerable.Repeat(member, 1)
                    : (doc.Members ?? Enumerable.Empty<Member_Document>());
            ...
            switch (name)
            {
                ...
                case "tenantid":
                    ...
                    break;
                case "gender":
                    foreach (var m in member != null ? Enumerable.Repeat(member, 1) : (doc.Members ?? Enumerable.Empty<Member_Document>()))
                        values.Add(m.Gender);
                    break;
                case "ageinyears":
                    foreach (var m in member != null ? Enumerable.Repeat(member, 1) : (doc.Members ?? Enumerable.Empty<Member_Document>()))
                        values.Add(m.AgeInYears);
                    break;
                case "ageinmonths":
                    foreach (var m in member != null ? Enumerable.Repeat(member, 1) : (doc.Members ?? Enumerable.Empty<Member_Document>()))
                        values.Add(m.AgeInMonths);
                    break;
                case "ageindays":
                    foreach (var m in member != null ? Enumerable.Repeat(member, 1) : (doc.Members ?? Enumerable.Empty<Member_Document>()))
                        values.Add(m.AgeInDays);
                    break;
                case "householdrole":
                    foreach (var m in member != null ? Enumerable.Repeat(member, 1) : (doc.Members ?? Enumerable.Empty<Member_Document>()))
                ...
                case "middlename":
                    ...
                    break;
                default:
                    // Computed fields: check household-level then member-level
        ...
        private static bool IsMemberFieldName(string? fieldName)
        {
            if (string.IsNullOrWhiteSpace(fieldName))
                return false;

            var normalized = fieldName.ToLowerInvariant().Replace("_", "");
            if (Regex.IsMatch(fieldName, @"^survey_(\d+)_q_(\d+)$", RegexOptions.IgnoreCase))
                return true;
        ...
        private static bool IsMemberScopedRule(CriteriaNode node)
        {
            if (node == null) return false;
            if (!string.IsNullOrEmpty(node.FieldName))
                return IsMemberFieldName(node.FieldName);

            if (node.Rules == null || node.Rules.Count == 0)
                return false;

            return node.Rules.Any(IsMemberScopedRule);
        }
        ...
        /// <summary>SQL-like pattern: % = any chars, _ = single char.</summary>
