...
public class VH_Dataset extends RecyclerView.ViewHolder implements CollectionUnitAdapter.ConstraintValidator, CollectionUnitAdapter.DatasetListener{
    ...
    private CollectionUnitAdapter.OptionsProvider optionsProvider;

    public VH_Dataset(View v) {
        super(v);
    ...
    public void bind(Map<Integer, CollectionUnitAdapter.ConstraintValidator> viewHolders, Map<Integer, Boolean> vhVisibilityById, Map<Integer, CollectionUnit> questionTypes, Map<Integer, String> answers, CollectionUnitAdapter.DatasetColumnsProvider datasetColumnsProvider, CollectionUnitAdapter.OptionsProvider optionsProvider, CollectionUnitAdapter.AnswerListener listener, CollectionUnit dp, boolean showOrder)
    {
    ...
    private boolean validateRow(TableRow row) {
        for (int i = 0; i < row.getChildCount(); i++) {
            ...
            FieldMeta meta = (FieldMeta) input.getTag();
    ...
    private void onSubmitAll() {
        ...
        answers.put(selected, jsonString);
...
// --- Add the Remove button as the LAST element ---
    ...
    private void setupTextField(View root, DatasetColumn col) {
        ...
        etValue.setTag(new FieldMeta(col.id, col.type, col.required, col.lookupId));
    }
    ...
    private void setupDateField(View root, DatasetColumn col) {
        ...
        etDate.setTag(new FieldMeta(col.id, col.type, col.required, col.lookupId));
    ...
    private void setupSwitchField(View root, DatasetColumn col) {
        ...
        tvTitle.setText(buildTitle(col.title, col.required));
        swValue.setTag(new FieldMeta(col.id, col.type, col.required, col.lookupId));
    }
    ...
    private void setupSpinnerField(View root, DatasetColumn col) {
        ...
        spValue.setTag(new FieldMeta(col.id, col.type, col.required, col.lookupId));
    }
    ...
    // Holder we’ll attach to each input view
