...
namespace BRaVe_Portal.Services
{
    ...
    /// Converts expression strings to JSON format matching ComputedExpressionDefinition.
    /// </summary>
    public static class ComputedExpressionParserImproved
    {
        ...
        public static string ParseToJson(List<IndicatorDto> indicators, string expression, IndicatorDataType outputDataType)
        {
            ...
            var definition = new ComputedExpressionDefinition
            {
                ...
                {
                    Field = field,
                    Type = GetComputedInputType(field, indicatorMap),
        ...
        private static string GetComputedInputType(string fieldName, Dictionary<string, IndicatorDataType> indicators)
        {
            ...
            return MapDataTypeToJsonType(indicators.GetValueOrDefault(fieldName, IndicatorDataType.Number));
        ...
        private class ExpressionParser
        {
            ...
            private ComputedExpressionBase ParseTernary()
            {
                ...
                // Check for standalone comparison expression (including AND/OR chains)
                var logicalComparison = TryParseLogicalComparison();
                if (logicalComparison != null)
                {
            ...
            private ComputedExpressionBase? TryParseLogicalComparison()
            {
                var savedPosition = _position;
                var left = TryParseStandaloneComparison();
                if (left == null)
                {
                ...
                ComputedExpressionBase current = left;
                while (true)
                {
                    ...
                    SkipWhitespace();
                    var right = TryParseStandaloneComparison();
                    if (right == null)
                    {
                        throw new ParseException($"Expected comparison expression after {logicalOp} at position {_position}");
                    }

                    current = new FunctionExpression
                    {
                        Name = logicalOp,
            ...
            {
                if (_position + word.Length > _expression.Length)
                    return false;

                var segment = _expression.Substring(_position, word.Length);
                if (!string.Equals(segment, word, StringComparison.OrdinalIgnoreCase))
                    return false;

                var before = _position > 0 ? _expression[_position - 1] : '\0';
                var afterPos = _position + word.Length;
                var after = afterPos < _expression.Length ? _expression[afterPos] : '\0';

                if ((before != '\0' && (char.IsLetterOrDigit(before) || before == '_')) ||
                    (after != '\0' && (char.IsLetterOrDigit(after) || after == '_')))
                {
                    return false;
                }
