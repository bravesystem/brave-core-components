...
{
    public class ROHQUpdateAssessmentModel : PageModel
    {
        ...
        public ROHQUpdateAssessmentModel(ILogger<IndexModel> logger, IAppCache cache, IRestApiService api, ILookupService lookupService, IRBAHelperService rBAHelperService)
        {
            _logger = logger;
            _cache = cache;
            _api = api;
        ...
        public async Task<IActionResult> OnGet(int assessmentId, AssessmentStatus statusId)
        {
            try
            {
                ...
                Recommendation? recommendation = await _rBAHelperService.GetCurrentRecommendation(User.Identifier(), false, assessmentId);
