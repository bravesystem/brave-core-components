...
{
    public class CoMUpdateAssessmentModel : PageModel
    {
        ...
        public CoMUpdateAssessmentModel(ILogger<IndexModel> logger, IAppCache cache, IRestApiService api, ILookupService lookupService, IRBAHelperService rBAHelperService)
        {
            _logger = logger;
            _cache = cache;
            _api = api;
        ...
        public async Task<IActionResult> OnGet(int assessmentId, AssessmentStatus statusId)
        {
            try
            {
                ...
                Decision? coMDecision = await _rBAHelperService.GetCurrentCoMDecision(User.Identifier(), assessmentId);


                if (coMDecision!=null)
                {

                    Decision = coMDecision.DecisionStatusId;
                    Note = coMDecision.Note;
                }
        ...
        public async Task<IActionResult> OnPostSubmitDecision(int assessmentId)
        {
            try
            {
                ...
                if (customValidationResult.IsValid)
                {
                    try
                    {
                        (bool success, string message) = await _rBAHelperService.SaveCoMDecision(User.Identifier(), comDecision);
        ...
        private CustomValidationResult Validate(DecisionDto coMDecisionDto)
        {
            ...
            //    {
            //        IsValid = false,
            //        Message = "Draft note saved. Decision is still pending."
            //    };
