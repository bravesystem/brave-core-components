...
var isLocalOnly = !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("LOCAL_ONLY") ?? string.Empty);
...
if (isPartner && !string.Equals(appEnvironment, "PARTNER", StringComparison.OrdinalIgnoreCase))
{
    Console.WriteLine(
        $"WARNING: Hostname '{websiteHostname}' indicates partner portal but Environment is '{(string.IsNullOrEmpty(appEnvironment) ? "(not set)" : appEnvironment)}'. Treating as PARTNER.");
    appEnvironment = "PARTNER";
}

builder.Services.AddSingleton(appEnvironment);
builder.Services.AddSingleton(typeof(bool), isLocalOnly);
...
Log.Information(
    "Portal startup: Environment={Environment}, Hostname={Hostname}, IsPartner={IsPartner}, LocalOnly={LocalOnly}",
    appEnvironment,
    websiteHostname,
    isPartner,
    isLocalOnly);
