...
namespace BRaVe_Mobile_Backend.Services
{
    public class SqlDataLoaderService : IDataLoaderService
    {
        ...
        {
            _connectionString = secretProvider.GetSecretAsync(Sql.PrimaryConnection);
            _bus = bus;
            _logger = logger;
        }

        public async void Load(Guid jobId)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add("@Id", SqlDbType.UniqueIdentifier).Value = jobId;
                await cn.OpenAsync();
        ...
        private async void ProcessAll(List<RegistrationActivityStaging> stagingRecords)
        {
            ...
            {
                _bus.SendMessageAsync(stagingRecords.FirstOrDefault().BatchId.ToString());
            }
        ...
        private async Task ProcessDistribution(Guid batchId, int tenantId, string activityCode, string payload)
        {
            ...
            DistributionAssistance distributionAssistance = JsonSerializer.Deserialize<DistributionAssistance>(payload);
        ...
        private async void ProcessConsent(Guid batchId, int tenantId, string activityCode, string payload)
        {
            ...
            ConsentFeedbackDataBundle dataBundle =
               JsonSerializer.Deserialize<ConsentFeedbackDataBundle>(feedback.Data);
            ...
            cmd.Parameters.Add("@ActivityCode", SqlDbType.VarChar, 50).Value = activityCode;
            cmd.Parameters.Add("@TenantId", SqlDbType.Int).Value = tenantId;
            cmd.Parameters.Add("@Payload", SqlDbType.NVarChar, -1).Value = JsonSerializer.Serialize( dataBundle.getData() );
            cmd.Parameters.Add("@Type", SqlDbType.Int).Value = dataBundle.type;
            cmd.Parameters.Add("@ConsentNotProvided", SqlDbType.Bit).Value = dataBundle.consentNotProvided;
        ...
        private async void ProcessHousehold(Guid batchId, int tenantId, string activityCode, string payload)
        {
